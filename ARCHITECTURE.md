...
**核心原则：把「会话/agent 的生命周期」与「浏览器连接的生命周期」彻底解耦。**
连接只是观察窗口，断了重连仍在；agent 与任务跑在服务端，与连接无关
（同 Claude.ai/ChatGPT：关页面回答照样生成，重连可见）。

### 三个对不上的时间尺度

```
对话/消息     —— 秒级，要即时反馈
工具调用      —— 有的秒级(write_netlist)，有的数分钟~数小时(run_optimizer 跑上百次仿真)
浏览器连接    —— 随时可能断（关页面/断网/切后台）
```

### 架构：会话 = 服务端常驻 actor + 事件日志

```
┌──────── Session（服务端常驻，不依赖任何连接）────────────────┐
│  拥有：对话状态 + agent 循环 + sandbox + 运行中的 jobs        │
│  agent 循环在此跑，调长任务时 await（服务端 asyncio，非阻塞） │
│  每个进展 append 到「事件日志」(append-only)                  │
└───────────────────────┬──────────────────────────────────────┘
                        │ WebSocket = 订阅该会话事件流（可断可重连）
                        ▼
   浏览器：连上 → 收当前快照 + 重放未读事件 → 之后收实时事件
            断线重连 → 从 last_seen 重放，不丢进度
```

### 长工具调用 = 沙箱里的 job

```
agent 调 run_optimizer → 沙箱起后台 job（返回 job_id）
  job: queued → running →（不断 emit 进度 → 事件日志 → WS → 浏览器）→ done/failed/cancelled
  用户点「停止」→ cancel job（给优化器发停止信号，优雅退出）
agent 循环 await job 完成，拿结果继续
```

**关键：job 归 session/sandbox 所有，不归 WebSocket 连接** —— 关页面 job 照跑，
重连看到当前状态并续上进度。

### 架构承诺

1. 会话/agent 与连接解耦 —— agent 跑在服务端常驻任务，WS 只是视图。
2. 每会话 append-only 事件日志 —— 既驱动实时流，又支持断线重放。
3. job 在沙箱跑、服务端跟踪、可取消。
4. 进度可恢复 —— 重连从 last_seen 重放。

### 持久化（MVP 决策）

- **MVP：内存为主 + job 续跑** —— 会话/事件先放内存，后端重启会丢；但 job 在沙箱内
  独立续跑。
- **架构预留可插拔持久化接口** —— 会话/消息/job/事件的存储抽象成接口，以后换 DB
  实现即可（后端重启不丢），不改上层逻辑。详见持久化数据模型（§9 第二层第 6 点，待议）。
